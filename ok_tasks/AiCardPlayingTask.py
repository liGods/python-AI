...
from ok_tasks.card_ai.heroes import HERO_REGISTRY, PASSIVE_OWNED_HEROES, T0_HEROES, normalize_hero_name as normalize_registered_hero_name  # 选将仍限账号列表，牌局内识别覆盖全65武将。
...
def choose_hero_candidate(candidates, statistics, exploration_games=10, preferred=None):  # 从三个OCR候选中先执行用户偏好，再优先安全可自动化的T0武将。
    ...
    return statistics.choose(selection_pool, exploration_games) if selection_pool else None  # 没有安全T0时保持原有被动候选探索与平滑胜率选择。
