...
from ok_tasks.card_ai.heroes import HERO_REGISTRY, PASSIVE_OWNED_HEROES, normalize_hero_name as normalize_registered_hero_name  # 选将仍限账号列表，牌局内识别覆盖全65武将。
...
def choose_hero_candidate(candidates, statistics, exploration_games=10, preferred=None):  # 从三个OCR候选中优先选择全被动技能武将。
    recognized = [hero for hero in candidates if hero is not None]  # 保留左中右顺序并过滤无法确认的名称。
    passive_candidates = [hero for hero in recognized if hero in PASSIVE_OWNED_HEROES]  # 只收集当前账号已经验证为全被动技能的候选。
    selection_pool = passive_candidates or recognized  # 有被动英雄时排除所有交互英雄，否则维持现有可用英雄兜底。
    if preferred in passive_candidates:  # 指定偏好只有同时属于本轮被动候选时才具有最高优先级。
        return preferred  # 返回用户偏好的被动英雄。
    return statistics.choose(selection_pool, exploration_games) if selection_pool else None  # 在被动候选中按冷启动局数和平滑胜率选择。
...
class AiCardPlayingTask(MaterialCollectorTask):  # 定义由训练模型决策的自动打牌任务。
    def __init__(self, *args, **kwargs):  # 初始化任务名称和 AI 专用配置。
        ...
        self.description = "只在合法动作中选择出牌；质量达标后自动使用稳定AI或10%小流量候选，否则回退规则算法。"  # 说明安全模型门禁和回退行为。
        self.default_config.update({  # 注册训练模型与识牌相关配置。
            "AI Adapter": "ok_tasks/RlCardRuleModel.py",  # 默认使用已经过实战验证的完整合法动作与身份策略；训练模型仅保留为影子评估。
            "AI Weights": "",  # 留空时从 data/card_ai/models 原子注册表读取稳定模型。
            "Use Model Registry": True,  # 即使旧配置仍保存规则适配器，也默认迁移到稳定/候选/回滚注册表。
            ...
            "Train Neural Candidates": True,  # 默认在每次任务结束时检查神经模型训练门槛。
            "Neural Minimum Games": 200,  # 至少累计二百局真实结算后才训练候选模型。
            "Neural Minimum Decisions": 2000,  # 至少累计两千个完整合法决策后才训练候选模型。
            "Search Budget Ms": 300,  # 困难局面在合法动作上执行信息集搜索的默认时间预算。
        })  # 完成 AI 默认配置注册。
        self.config_description.update({  # 为 GUI 配置补充中文说明。
            "AI Adapter": "实现 load_model 和 predict 的 Python 适配脚本路径。",  # 说明模型框架通过适配器接入。
            "AI Weights": "留空使用 data/card_ai/models；当前适配器只会加载已晋升的 OpenVINO 稳定模型。",  # 说明模型注册表行为。
            "Use Model Registry": "启用时使用高容量模型注册表；关闭后才按 AI Adapter 配置强制使用其他适配器。",  # 说明旧配置迁移方式。
            ...
            "Train Neural Candidates": "任务结束时检查数据门槛并训练 OpenVINO 合法动作评分候选。",  # 说明训练不会直接替换稳定规则。
            "Neural Minimum Games": "神经候选模型训练所需的最少完整结算局数。",  # 说明整局训练门槛。
            "Neural Minimum Decisions": "神经候选模型训练所需的最少成功决策数量。",  # 说明决策训练门槛。
            "Search Budget Ms": "稳定神经模型的困难残局搜索预算，范围0到1200毫秒；超时自动使用神经评分。",  # 说明搜索安全回退。
        })  # 完成 AI 配置说明注册。
    ...
    def _prepare_session(self):  # 初始化采集状态和训练模型会话。
        ...
        try:  # 捕获模型缺失或适配接口错误以避免任务崩溃。
            if self.config.get("Use Model Registry", True):  # 默认将旧版规则配置迁移到带安全门禁的模型注册表。
                adapter_path = Path("ok_tasks/HighCapacityCardModel.py")  # 高容量适配器内部在门禁失败时回退稳定规则。
            else:  # 用户明确关闭注册表时保留自定义适配器能力。
                adapter_path = Path(self.config.get("AI Adapter", "ok_tasks/RlCardRuleModel.py"))  # 读取用户指定的适配脚本配置。
            configured_weights = str(self.config.get("AI Weights", "")).strip()  # 读取并清理可选训练权重路径。
            weights_path = Path(configured_weights) if configured_weights else None  # 规则模型留空时不执行权重文件检查。
            self.ai_model = TrainedModelAdapter(adapter_path, weights_path).load()  # 加载训练模型并准备推理。
            source_name = str(weights_path) if weights_path is not None else adapter_path.name  # 生成当前决策来源的可读名称。
            ...
            model_value = getattr(self.ai_model, "model", None)  # 读取高容量适配器公开的质量门禁状态。
            if isinstance(model_value, dict) and model_value.get("quality_gate"):  # 只有新适配器提供门禁时显示。
                quality = model_value["quality_gate"]  # 获取本次启动锁定的运行质量快照。
                self.info_set("Neural Gate", "通过" if quality.get("passed") else "规则回退")  # 实时说明神经模型是否允许启用。
    ...
    def _finalize_session(self, status):  # 在任务结束后分析策略表现并生成包含优化结果的报告。
        ...
        if self.config.get("Train Neural Candidates", True):  # 保留旧配置开关，但不在Windows自动化进程中占用GPU训练。
            neural = {"trained": False, "message": "真实日志已保留；高容量候选请在WSL2运行 python -m ok_tasks.card_ai continuous"}  # 将训练交给可恢复的多日循环。
        else:  # 用户关闭本地训练时保留稳定规则算法。
            neural = {"trained": False, "message": "神经候选训练已关闭"}  # 记录明确关闭状态供报告展示。
        optimization["neural"] = neural  # 将神经候选训练结果合并到机器可读优化报告。
        optimization["message"] = f"{optimization.get('message', '规则策略未执行')}；{neural.get('message', '神经训练未执行')}"  # 生成同时覆盖规则和神经管线的中文总结。
