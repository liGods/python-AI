...
from rlcard.games.doudizhu.utils import CARD_TYPE, ID_2_ACTION, INDEX, contains_cards  # 复用 RLCard 官方斗地主动作空间与牌型定义。
...
INTERNAL_TO_RLCARD = {"X": "B", "D": "R"}  # 将项目大小王编码转换成 RLCard 的黑王和红王编码。
RLCARD_TO_INTERNAL = {"B": "X", "R": "D"}  # 将 RLCard 动作重新转换成项目内部编码。
_ACTION_ORDER = {action: index for index, action in enumerate(ID_2_ACTION)}
_WILDCARD = "W"
_CARD_SORT_ORDER = {card: index for index, card in enumerate(tuple(INDEX) + (_WILDCARD,))}
...
def _to_rlcard(cards):  # 将项目牌组转换成 RLCard 要求的有序字符串。
    converted = [INTERNAL_TO_RLCARD.get(card, card) for card in cards]  # 逐张转换大小王并保留普通点数。
    return "".join(sorted(converted, key=lambda card: _CARD_SORT_ORDER.get(card, len(_CARD_SORT_ORDER))))  # 万能牌排在标准点数后，避免进入 RLCard 前就触发 KeyError。


def _to_internal(action):  # 将 RLCard 动作字符串转换回项目牌组列表。
    return [RLCARD_TO_INTERNAL.get(card, card) for card in action]  # 逐张还原大小王编码供屏幕点击映射。


def _contains_with_wildcards(hand, action):  # 判断实体手牌能否通过万能牌补齐一个 RLCard 生效动作。
    natural = Counter(card for card in hand if card != _WILDCARD)  # 自然牌优先承担动作中的同点数需求。
    required = Counter(action)  # RLCard 动作只包含已经选定的生效点数。
    missing = sum(max(0, count - natural[rank]) for rank, count in required.items())  # 每个缺口消耗一张万能牌。
    return missing <= hand.count(_WILDCARD)  # 未使用的万能牌可以继续留在手中。


def _physical_action(hand, effective_action):  # 把生效动作还原成屏幕上实际需要点击的实体牌。
    remaining_natural = Counter(card for card in hand if card != _WILDCARD)  # 同点数自然牌先于万能牌使用，与引擎卡牌映射一致。
    physical = []
    wildcard_count = hand.count(_WILDCARD)
    for rank in effective_action:
        if remaining_natural[rank] > 0:
            ...
            wildcard_count -= 1
            physical.append(_WILDCARD)
        else:
            raise ValueError(f"生效动作无法映射回实体手牌: {effective_action}")
    return "".join(sorted(physical, key=lambda card: _CARD_SORT_ORDER.get(card, len(_CARD_SORT_ORDER))))


def _legal_action_variants(hand, target):  # 同时返回 RLCard 生效动作与可点击实体动作。
    return tuple((action, _physical_action(hand, action)) for action in _legal_actions(hand, target))


def _is_five_bomb(action):  # 判断百将牌技能增牌后形成的五张同点数超级炸弹。
    return len(action) == 5 and len(set(action)) == 1  # 五张实体牌点数完全相同即为五炸，强度高于王炸。


def _action_beats(candidate, target):  # 使用 RLCard 牌型和权重判断候选动作能否压过桌面动作。
    if _is_five_bomb(candidate):  # 五炸可以跨牌型压制普通牌、四炸和王炸。
        return not _is_five_bomb(target) or INDEX[candidate[0]] > INDEX[target[0]]  # 五炸互压时仍按点数比较。
    if _is_five_bomb(target):  # 任何标准牌型包括王炸都不能反压桌面五炸。
        return False  # 只能由更大的五炸响应。
    candidate_types = CARD_TYPE[0].get(candidate, [])  # 读取候选动作可能对应的全部标准牌型解释。
    target_types = CARD_TYPE[0].get(target, [])  # 读取桌面动作可能对应的全部标准牌型解释。
    for candidate_type, candidate_weight in candidate_types:  # 检查候选动作的每一种合法牌型解释。
        for target_type, target_weight in target_types:  # 与桌面动作的每一种合法牌型解释进行比较。
            if candidate_type == "rocket":  # 王炸可以压过除王炸外的任意动作。
                return target_type != "rocket"  # 桌面也是王炸时没有更大的动作。
            if candidate_type == "bomb" and target_type not in {"bomb", "rocket"}:  # 普通炸弹可以压过所有非炸弹牌型。
                return True  # 接受跨牌型的炸弹响应。
            if candidate_type == target_type and int(candidate_weight) > int(target_weight):  # 同牌型只允许更高主点数压制。
                return True  # 接受长度与结构一致且点数更高的动作。
    return False  # 其他不同牌型、较小点数或错误长度均不能跟牌。


@lru_cache(maxsize=512)  # 缓存最近牌局状态以降低两万余动作空间的重复扫描开销。
def _legal_actions(hand, target):  # 从 RLCard 完整动作空间中生成当前屏幕状态的合法动作。
    counts = Counter(hand)  # 技能复制、回收和补牌可能让同一点数超过标准牌库四张上限。
    wildcard_count = counts[_WILDCARD]
    if wildcard_count:
        standard_actions = [
            action
            for action in ID_2_ACTION
            if action != "pass" and _contains_with_wildcards(hand, action)
        ]  # 万能牌直接补动作缺口，避免对十五种替代值做指数级笛卡尔展开。
    elif max(counts.values(), default=0) <= 4:
        playable = DoudizhuJudger.playable_cards_from_hand(hand)
        standard_actions = sorted((action for action in playable if action in _ACTION_ORDER), key=_ACTION_ORDER.__getitem__)  # 技能可生成重复王；RLCard 会返回 RR/BB 等非标准对子，必须过滤。
    else:
        standard_actions = [
            action for action in ID_2_ACTION if action != "pass" and contains_cards(hand, action)
        ]
    actions = [action for action in standard_actions if not target or _action_beats(action, target)]
    for rank in INDEX:  # 按点数从低到高补充 RLCard 标准动作空间没有的五炸。
        if counts[rank] + wildcard_count >= 5:  # 自然牌与万能牌合计达到五张时可以组成对应生效点数的超级炸弹。
            action = rank * 5  # 生成可由现有计数扣除和屏幕映射共同处理的规范动作。
            if action not in actions and (not target or _action_beats(action, target)):  # 主动可出，跟牌时按五炸最高规则比较。
                actions.append(action)  # 加入完整合法候选集合供规则、神经模型和搜索共同使用。
    return tuple(actions)  # 返回不可变结果供缓存和确定性评分复用。
...
def choose_skill_card_pick(hand_cards, option_cards, hero=None):  # 模拟获得每张展示牌后的完整牌路并决定获取或取消技能。
    ...
    best = max(candidates, key=lambda item: (item["utility"], -item["after_turns"], -item["after_solos"], INDEX[INTERNAL_TO_RLCARD.get(item["card"], item["card"]) ]))  # 选择净收益最高且牌路更紧凑的展示牌。
...
_to_rlcard = _adapter_to_rlcard
_to_internal = _adapter_to_internal
_legal_actions = _adapter_legal_actions
...
def evaluate_guan_yinping_action(hand, action, hero_state=None):  # 模拟花武随机获得 J、Q、K 后的真实残局，而不是把四张以上动作一律当成收益。
    ...
    for gained_rank in ("J", "Q", "K"):  # 花武只会从三个指定人头牌中随机获得一张。
        augmented_hand = "".join(sorted(hand + gained_rank, key=lambda card: _CARD_SORT_ORDER.get(card, len(_CARD_SORT_ORDER))))  # 同一获得点数的完整动作空间可被所有候选共享缓存，兼容手中尚未使用的万能牌。
        outcome_hand = "".join(sorted(remaining + gained_rank, key=lambda card: _CARD_SORT_ORDER.get(card, len(_CARD_SORT_ORDER))))  # 构造该随机分支下实际留下的完整手牌，避免万能牌进入 RLCard 索引。
...
def _score_action(hand, action, urgent, hero=None, last_action_type=None, policy_id="balanced", target="", enemy_counts=None, hero_state=None, pressure_context=None, skill_projection=None, hero_context=None, physical_action=None, action_type=None):  # 为一个合法生效动作生成完整技能结算后的确定性评分，同时兼容旧调用签名。
...
def _build_candidate_records(hand, target, enemy_counts, hero=None, last_action_type=None, policy_id="balanced", hero_state=None, pressure_context=None, decision_context=None):  # 一次构建候选、英雄投影和评分，供解释与最终选牌共同消费。
    ...
    for effective_action, physical_action in _legal_action_variants(hand, target):
        ...
        if previous is None or (record["score"], _ACTION_ORDER.get(effective_action, len(_ACTION_ORDER)), effective_action) < (previous["score"], _ACTION_ORDER.get(previous["action"], len(_ACTION_ORDER)), previous["action"]):
