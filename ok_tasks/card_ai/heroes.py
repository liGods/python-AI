...
SIMULATED_HEROES = (  # 仅保留已经接入权威状态机的十四名武将，不能因账号新增拥有角色而自动提升验证状态。
    "典韦",
    "夏侯惇",
    "关羽",
    "张飞",
...
def registry_contract_errors() -> tuple[str, ...]:
    ...
    if len(AUTHORITATIVE_RULE_IDS) != 18:
        errors.append(f"expected 18 authoritative rules, found {len(AUTHORITATIVE_RULE_IDS)}")
